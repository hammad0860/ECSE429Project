...
class TodoApiUnitTests(unittest.TestCase):
    ...
    def setUp(self):
        global EXPECTED_JSON_ALL_IDS, GET_TODO_CATEGORIES_RESPONSE

        self.expected_json_all_ids = EXPECTED_JSON_ALL_IDS
        self.get_todo_categories_response = GET_TODO_CATEGORIES_RESPONSE
    ...
    # Gets all categories for a todo
    def test_get_todo_categories(self):
        response = requests.get(f"{BASE_URL}{TODOS_ENDPOINT}/categories", headers={"Accept": "application/json"})
        self.assertEqual(response.status_code, 200)
        actual_json = response.json()
        diff = DeepDiff(actual_json, self.get_todo_categories_response, ignore_order=True)
